...
init_i18n() {
  ...
  # 语音识别模块
  I18N["voice_module_title_cn"]="语音识别模块安装"
  I18N["voice_module_title_en"]="Voice Recognition Module Installation"
  I18N["voice_enable_ask_cn"]="是否启用语音识别功能？（输入 y/Y/yes 启用，其他跳过）"
  I18N["voice_enable_ask_en"]="Enable voice recognition? (Enter y/Y/yes to enable, others to skip)"
  I18N["voice_skip_cn"]="跳过语音识别模块安装"
  I18N["voice_skip_en"]="Skipping voice recognition module installation"
  I18N["voice_termux_api_title_cn"]="安装 Termux:API 应用"
  I18N["voice_termux_api_title_en"]="Install Termux:API Application"
  I18N["voice_termux_api_desc_cn"]="请下载并安装 Termux:API 应用，然后授予麦克风权限"
  I18N["voice_termux_api_desc_en"]="Please download and install Termux:API app, then grant microphone permission"
  I18N["voice_termux_api_url_cn"]="下载地址："
  I18N["voice_termux_api_url_en"]="Download URL:"
  I18N["voice_termux_api_steps_cn"]="安装步骤："
  I18N["voice_termux_api_steps_en"]="Installation Steps:"
  I18N["voice_termux_api_step1_cn"]="下载并安装 termux-api-app_xxx+github.debug.apk"
  I18N["voice_termux_api_step1_en"]="Download and install termux-api-app_xxx+github.debug.apk"
  I18N["voice_termux_api_step2_cn"]="桌面长按 Termux:API 图标，打开应用信息"
  I18N["voice_termux_api_step2_en"]="Long press Termux:API icon on desktop, open app info"
  I18N["voice_termux_api_step3_cn"]="进入权限，将麦克风权限设置为允许"
  ...
  I18N["voice_config_title_en"]="Configure AI Voice Recognition Parameters"
  I18N["voice_config_base_url_cn"]="AI 语音识别 API 基础地址"
  I18N["voice_config_base_url_en"]="AI Voice Recognition API Base URL"
  I18N["voice_config_model_cn"]="AI 语音识别模型"
  I18N["voice_config_model_en"]="AI Voice Recognition Model"
  I18N["voice_config_apikey_cn"]="AI 语音识别 API Key"
  I18N["voice_config_apikey_en"]="AI Voice Recognition API Key"
  I18N["voice_config_max_duration_cn"]="最长录音时间（秒）"
...
export VOICE_API_BASE_URL=""
export VOICE_API_MODEL=""
export VOICE_API_KEY=""
export VOICE_MAX_DURATION=""
EOF
...
install_voice_module() {
  echo
  echo -e "${PURPLE}╔══════════════════════════════════════════════════════════════╗${NC}"
  echo -e "${PURPLE}║${NC}              ${BOLD}$(i18n voice_module_title)${NC}                              ${PURPLE}║${NC}"
  echo -e "${PURPLE}╚══════════════════════════════════════════════════════════════╝${NC}"
  echo
  ...
  read -rp "$(i18n voice_enable_ask): " voice_enable
  case "$voice_enable" in
    [Yy]|[Yy][Ee][Ss])
      ;;
    *)
  ...
  # 第1步：提示安装 Termux:API
  echo
  echo -e "${YELLOW}━━━ $(i18n voice_termux_api_title) ━━━${NC}"
  echo
  echo -e "${CYAN}$(i18n voice_termux_api_desc)${NC}"
  echo
  echo -e "${BLUE}$(i18n voice_termux_api_url)${NC}"
  echo -e "  ${GREEN}https://github.com/termux/termux-api/releases/${NC}"
  echo
  echo -e "${BLUE}$(i18n voice_termux_api_steps)${NC}"
  echo -e "  ${GREEN}1.${NC} $(i18n voice_termux_api_step1)"
  echo -e "  ${GREEN}2.${NC} $(i18n voice_termux_api_step2)"
  echo -e "  ${GREEN}3.${NC} $(i18n voice_termux_api_step3)"
  ...
  # 第2步：安装 termux-api
  echo
  log_info "$(i18n voice_installing_api)"
  pkg install termux-api -y
  
  # 第3步：配置语音识别参数
  echo
  echo -e "${CYAN}━━━ $(i18n voice_config_title) ━━━${NC}"
  echo
  echo -e "${YELLOW}$(i18n config_tip)${NC}"
  echo
  
  local DEFAULT_VOICE_BASE_URL="https://api.siliconflow.cn/v1"
  local DEFAULT_VOICE_MODEL="FunAudioLLM/SenseVoiceSmall"
  local DEFAULT_VOICE_API_KEY="sk-your-key"
  local DEFAULT_VOICE_MAX_DURATION="60"
  
  read -rp "  $(i18n voice_config_base_url) [${DEFAULT_VOICE_BASE_URL}]: " voice_base_url
  voice_base_url=${voice_base_url:-$DEFAULT_VOICE_BASE_URL}
  
  read -rp "  $(i18n voice_config_model) [${DEFAULT_VOICE_MODEL}]: " voice_model
  voice_model=${voice_model:-$DEFAULT_VOICE_MODEL}
  
  read -rp "  $(i18n voice_config_apikey) [${DEFAULT_VOICE_API_KEY}]: " voice_api_key
  voice_api_key=${voice_api_key:-$DEFAULT_VOICE_API_KEY}
  
  read -rp "  $(i18n voice_config_max_duration) [${DEFAULT_VOICE_MAX_DURATION}]: " voice_max_duration
  voice_max_duration=${voice_max_duration:-$DEFAULT_VOICE_MAX_DURATION}
  
  # 第4步：保存配置
  # 更新配置文件
  sed -i 's/^export VOICE_ENABLED=.*/export VOICE_ENABLED="true"/' ~/.autoglm/config.sh
  sed -i "s|^export VOICE_API_BASE_URL=.*|export VOICE_API_BASE_URL=\"$voice_base_url\"|" ~/.autoglm/config.sh
  sed -i "s|^export VOICE_API_MODEL=.*|export VOICE_API_MODEL=\"$voice_model\"|" ~/.autoglm/config.sh
  sed -i "s|^export VOICE_API_KEY=.*|export VOICE_API_KEY=\"$voice_api_key\"|" ~/.autoglm/config.sh
  sed -i "s|^export VOICE_MAX_DURATION=.*|export VOICE_MAX_DURATION=\"$voice_max_duration\"|" ~/.autoglm/config.sh
  
  echo
  log_succ "$(i18n voice_module_installed)"
}
...
init_i18n() {
  ...
  I18N["menu_voice_cn"]="🎤 使用语音识别启动"
  I18N["menu_voice_en"]="🎤 Start with Voice Recognition"
  ...
  I18N["menu_voice_config_cn"]="🎙️  修改语音识别配置"
  I18N["menu_voice_config_en"]="🎙️  Modify Voice Recognition Config"
  ...
  # 语音识别相关
  I18N["voice_mode_title_cn"]="🎤 语音识别模式"
  I18N["voice_mode_title_en"]="🎤 Voice Recognition Mode"
  I18N["voice_start_prompt_cn"]="输入 Y/y/yes 或直接回车开始录音，输入 exit/stop/quit 退出语音识别模式"
  I18N["voice_start_prompt_en"]="Enter Y/y/yes or press Enter to start recording, enter exit/stop/quit to exit voice mode"
  I18N["voice_stop_prompt_cn"]="正在录音... 输入 Y/y/yes 或直接回车停止录音，输入 exit/stop/quit 取消本次录音"
  I18N["voice_stop_prompt_en"]="Recording... Enter Y/y/yes or press Enter to stop, enter exit/stop/quit to cancel"
  I18N["voice_recording_cn"]="开始录音..."
  I18N["voice_recording_en"]="Start recording..."
  I18N["voice_recording_stop_cn"]="停止录音"
  I18N["voice_recording_stop_en"]="Stop recording"
  I18N["voice_cancelled_cn"]="已取消本次录音"
  I18N["voice_cancelled_en"]="Recording cancelled"
  I18N["voice_exit_cn"]="退出语音识别模式"
  I18N["voice_exit_en"]="Exiting voice recognition mode"
  I18N["voice_transcribing_cn"]="正在转换语音为文字..."
  I18N["voice_transcribing_en"]="Transcribing voice to text..."
  I18N["voice_transcribe_success_cn"]="语音识别成功"
  I18N["voice_transcribe_success_en"]="Voice transcription successful"
  I18N["voice_transcribe_fail_cn"]="语音识别失败"
  I18N["voice_transcribe_fail_en"]="Voice transcription failed"
  I18N["voice_task_text_cn"]="识别到的任务"
  I18N["voice_task_text_en"]="Recognized task"
  I18N["voice_executing_cn"]="正在执行任务..."
  I18N["voice_executing_en"]="Executing task..."
  I18N["voice_task_complete_cn"]="任务执行完成"
  I18N["voice_task_complete_en"]="Task completed"
  I18N["voice_not_enabled_cn"]="语音识别功能未启用，请先在部署时启用或重新运行 deploy.sh"
  I18N["voice_not_enabled_en"]="Voice recognition not enabled, please enable during deployment or re-run deploy.sh"
  I18N["voice_max_duration_cn"]="最长录音时间"
  I18N["voice_max_duration_en"]="Max recording duration"
  I18N["voice_seconds_cn"]="秒"
  I18N["voice_seconds_en"]="seconds"
  I18N["voice_config_title_cn"]="🎙️ 修改语音识别配置"
  I18N["voice_config_title_en"]="🎙️ Modify Voice Recognition Config"
  I18N["voice_api_url_cn"]="语音识别 API 地址"
  I18N["voice_api_url_en"]="Voice API URL"
  I18N["voice_model_cn"]="语音识别模型"
  I18N["voice_model_en"]="Voice Model"
  I18N["voice_apikey_cn"]="语音识别 API Key"
  I18N["voice_apikey_en"]="Voice API Key"
  I18N["voice_duration_cn"]="最长录音时间(秒)"
  ...
  I18N["voice_config_cn"]="语音识别配置"
  I18N["voice_config_en"]="Voice Recognition Config"
  I18N["voice_enabled_cn"]="语音识别"
  I18N["voice_enabled_en"]="Voice Recognition"
  I18N["enabled_cn"]="已启用"
  I18N["enabled_en"]="Enabled"
  I18N["disabled_cn"]="未启用"
  I18N["disabled_en"]="Disabled"
  ...
  I18N["uninstall_termux_api_cn"]="是否卸载 termux-api？"
  I18N["uninstall_termux_api_en"]="Uninstall termux-api?"
  I18N["termux_api_uninstalled_cn"]="termux-api 已卸载"
  I18N["termux_api_uninstalled_en"]="termux-api uninstalled"
...
# Voice Recognition Config
export VOICE_ENABLED="${VOICE_ENABLED:-false}"
export VOICE_API_BASE_URL="${VOICE_API_BASE_URL:-}"
export VOICE_API_MODEL="${VOICE_API_MODEL:-}"
export VOICE_API_KEY="${VOICE_API_KEY:-}"
export VOICE_MAX_DURATION="${VOICE_MAX_DURATION:-}"
EOF
  chmod +x "$CONFIG_FILE"
}
...
show_current_config() {
  ...
  if [[ "${VOICE_ENABLED:-false}" == "true" ]]; then
    echo -e "  ${BLUE}$(i18n voice_enabled)${NC}: ${GREEN}$(i18n enabled)${NC}"
  fi
  echo
}
...
show_main_menu() {
  ...
  local menu_idx=2
  if [[ "${VOICE_ENABLED:-false}" == "true" ]]; then
    echo -e "  ${GREEN}${menu_idx}.${NC} $(i18n menu_voice)"
    ((menu_idx++))
  fi
  
  echo -e "  ${GREEN}${menu_idx}.${NC} $(i18n menu_adb)"
  local adb_idx=$menu_idx
  ((menu_idx++))
  
  echo -e "  ${GREEN}${menu_idx}.${NC} $(i18n menu_config)"
  local config_idx=$menu_idx
  ((menu_idx++))
  
  if [[ "${VOICE_ENABLED:-false}" == "true" ]]; then
    echo -e "  ${GREEN}${menu_idx}.${NC} $(i18n menu_voice_config)"
    local voice_config_idx=$menu_idx
    ((menu_idx++))
  fi
  
  echo -e "  ${GREEN}${menu_idx}.${NC} $(i18n menu_apps)"
  local apps_idx=$menu_idx
  ((menu_idx++))
  
  echo -e "  ${GREEN}${menu_idx}.${NC} $(i18n menu_view_config)"
  local view_idx=$menu_idx
  ((menu_idx++))
  
  echo -e "  ${GREEN}${menu_idx}.${NC} $(i18n menu_lang)"
  local lang_idx=$menu_idx
  ((menu_idx++))
  
  echo -e "  ${GREEN}${menu_idx}.${NC} $(i18n menu_uninstall)"
  local uninstall_idx=$menu_idx
  
  echo -e "  ${GREEN}0.${NC} $(i18n menu_exit)"
  echo
  
  # 保存菜单索引供后续使用
  MENU_VOICE_IDX="${VOICE_ENABLED:-false}" == "true" && echo 2 || echo -1
  MENU_ADB_IDX=$adb_idx
  MENU_CONFIG_IDX=$config_idx
  MENU_VOICE_CONFIG_IDX=${voice_config_idx:-0}
  MENU_APPS_IDX=$apps_idx
  MENU_VIEW_IDX=$view_idx
  MENU_LANG_IDX=$lang_idx
  MENU_UNINSTALL_IDX=$uninstall_idx
}
...
modify_voice_config() {
  show_header
  echo -e "${CYAN}╔══════════════════════════════════════════════════════════════╗${NC}"
  echo -e "${CYAN}║${NC}              ${BOLD}$(i18n voice_config_title)${NC}                            ${CYAN}║${NC}"
  echo -e "${CYAN}╚══════════════════════════════════════════════════════════════╝${NC}"
  echo
  echo -e "${YELLOW}$(i18n keep_current)${NC}"
  echo
  ...
  read -rp "  $(i18n voice_api_url) [$VOICE_API_BASE_URL]: " new_val
  [[ -n "$new_val" ]] && VOICE_API_BASE_URL="$new_val"
  
  read -rp "  $(i18n voice_model) [$VOICE_API_MODEL]: " new_val
  [[ -n "$new_val" ]] && VOICE_API_MODEL="$new_val"
  
  read -rp "  $(i18n voice_apikey) [$VOICE_API_KEY]: " new_val
  [[ -n "$new_val" ]] && VOICE_API_KEY="$new_val"
  
  read -rp "  $(i18n voice_duration) [$VOICE_MAX_DURATION]: " new_val
  [[ -n "$new_val" ]] && VOICE_MAX_DURATION="$new_val"
  
  save_config
  
  echo
  echo -e "${GREEN}[SUCC]${NC} $(i18n voice_config_saved)"
  read -rp "$(i18n press_enter_main) "
}
...
view_config() {
  ...
  if [[ "${VOICE_ENABLED:-false}" == "true" ]]; then
    echo
    echo -e "${BLUE}$(i18n voice_config):${NC}"
    echo -e "┌────────────────────────┬────────────────────────────────────┐"
    printf "│ %-22s │ %-34s │\n" "VOICE_ENABLED" "$(i18n enabled)"
    printf "│ %-22s │ %-34s │\n" "VOICE_API_BASE_URL" "$VOICE_API_BASE_URL"
    printf "│ %-22s │ %-34s │\n" "VOICE_API_MODEL" "$VOICE_API_MODEL"
    printf "│ %-22s │ %-34s │\n" "VOICE_API_KEY" "${VOICE_API_KEY:0:20}..."
    printf "│ %-22s │ %-34s │\n" "VOICE_MAX_DURATION" "${VOICE_MAX_DURATION}s"
    echo -e "└────────────────────────┴────────────────────────────────────┘"
  fi
...
transcribe_audio() {
  ...
  if [[ "$http_code" == "200" ]]; then
    local text
    text=$(echo "$body" | grep -oP '"text"\s*:\s*"\K[^"]+' || echo "")
    if [[ -n "$text" ]]; then
      echo "$text"
      return 0
    else
...
voice_recognition_mode() {
  # 检查语音识别是否启用
  if [[ "${VOICE_ENABLED:-false}" != "true" ]]; then
    echo -e "${RED}[ERROR]${NC} $(i18n voice_not_enabled)"
    read -rp "$(i18n press_enter) "
    return 1
  fi
  ...
  local AUDIO_CACHE_DIR="$HOME/.autoglm"
  mkdir -p "$AUDIO_CACHE_DIR"
  
  show_header
  echo -e "${PURPLE}╔══════════════════════════════════════════════════════════════╗${NC}"
  echo -e "${PURPLE}║${NC}              ${BOLD}$(i18n voice_mode_title)${NC}                                  ${PURPLE}║${NC}"
  echo -e "${PURPLE}╚══════════════════════════════════════════════════════════════╝${NC}"
  echo
  echo -e "${CYAN}$(i18n voice_max_duration): ${GREEN}${VOICE_MAX_DURATION}${NC} $(i18n voice_seconds)${NC}"
  echo
  
  while true; do
    echo
    echo -e "${YELLOW}$(i18n voice_start_prompt)${NC}"
    read -rp "> " start_input
    
    case "$start_input" in
      exit|stop|quit|EXIT|STOP|QUIT)
        echo -e "${BLUE}[INFO]${NC} $(i18n voice_exit)"
        return 0
        ;;
      ""|[Yy]|[Yy][Ee][Ss])
        # 开始录音
        local audio_file="$AUDIO_CACHE_DIR/voice_$(date +%s).mp3"
        
        echo -e "${GREEN}[INFO]${NC} $(i18n voice_recording)"
        
        # 后台启动录音
        termux-microphone-record -e aac -l "${VOICE_MAX_DURATION}" -f "$audio_file" &
        local record_pid=$!
        
        echo
        echo -e "${YELLOW}$(i18n voice_stop_prompt)${NC}"
        read -rp "> " stop_input
        ...
        case "$stop_input" in
          ...
          ""|[Yy]|[Yy][Ee][Ss])
            ...
            kill $record_pid 2>/dev/null || true
            sleep 1
            echo -e "${BLUE}[INFO]${NC} $(i18n voice_recording_stop)"
            ;;
        esac
        ...
        echo -e "${GREEN}[SUCC]${NC} $(i18n voice_transcribe_success)"
        echo -e "${CYAN}$(i18n voice_task_text):${NC} ${GREEN}$task_text${NC}"
        echo
        
        # 执行单次任务
        echo -e "${BLUE}[INFO]${NC} $(i18n voice_executing)"
        ...
        cd "$AUTOGLM_DIR"
        python main.py "${CMD_ARGS[@]}" "$task_text"
        
        echo
        echo -e "${GREEN}[SUCC]${NC} $(i18n voice_task_complete)"
        ;;
      *)
        echo -e "${RED}$(i18n invalid_choice)${NC}"
        ;;
...
uninstall_full() {
  ...
  echo
  if ask_yes_no "$(i18n delete_cmd_ask)"; then
    rm -f "$HOME/bin/autoglm" 2>/dev/null || true
    rm -rf "$HOME/.autoglm" 2>/dev/null || true
    sed -i '/source ~\/.autoglm\/config.sh/d' "$HOME/.bashrc" 2>/dev/null || true
    sed -i '/source \$HOME\/.autoglm\/config.sh/d' "$HOME/.bashrc" 2>/dev/null || true
    echo -e "${GREEN}[SUCC]${NC} $(i18n cleaned_bashrc)"
    did_something=true
  ...
  if [[ -f "$HOME/.cargo/config.toml" ]]; then
    ...
    if ask_yes_no "$(i18n delete_cargo_ask)"; then
      rm -f "$HOME/.cargo/config.toml" "$HOME/.cargo/config" 2>/dev/null || true
  ...
  if in_termux; then
    ...
    if pkg list-installed 2>/dev/null | grep -q "termux-api"; then
      if ask_yes_no "$(i18n uninstall_termux_api)"; then
        pkg uninstall -y termux-api 2>/dev/null || true
        echo -e "${GREEN}[SUCC]${NC} $(i18n termux_api_uninstalled)"
        did_something=true
      fi
    fi
    echo
...
start_single_task() {
  ...
  cd "$AUTOGLM_DIR"
  python main.py "${CMD_ARGS[@]}" "$task_description"
}
...
main_menu_loop() {
  while true; do
    ...
    local next_idx=2
    local voice_idx=0
    local adb_idx config_idx voice_config_idx apps_idx view_idx lang_idx uninstall_idx
    
    if [[ "${VOICE_ENABLED:-false}" == "true" ]]; then
      echo -e "  ${GREEN}${next_idx}.${NC} $(i18n menu_voice)"
      voice_idx=$next_idx
      ((next_idx++))
    fi
    
    echo -e "  ${GREEN}${next_idx}.${NC} $(i18n menu_adb)"
    adb_idx=$next_idx
    ((next_idx++))
    
    echo -e "  ${GREEN}${next_idx}.${NC} $(i18n menu_config)"
    config_idx=$next_idx
    ((next_idx++))
    
    if [[ "${VOICE_ENABLED:-false}" == "true" ]]; then
      echo -e "  ${GREEN}${next_idx}.${NC} $(i18n menu_voice_config)"
      voice_config_idx=$next_idx
      ((next_idx++))
    fi
    
    echo -e "  ${GREEN}${next_idx}.${NC} $(i18n menu_apps)"
    apps_idx=$next_idx
    ((next_idx++))
    
    echo -e "  ${GREEN}${next_idx}.${NC} $(i18n menu_view_config)"
    view_idx=$next_idx
    ((next_idx++))
    
    echo -e "  ${GREEN}${next_idx}.${NC} $(i18n menu_lang)"
    lang_idx=$next_idx
    ((next_idx++))
    
    echo -e "  ${GREEN}${next_idx}.${NC} $(i18n menu_uninstall)"
    uninstall_idx=$next_idx
    
    echo -e "  ${GREEN}0.${NC} $(i18n menu_exit)"
    echo
    
    read -rp "$(i18n menu_select) [0-${uninstall_idx}]: " choice
    ...
    case "$choice" in
      ...
      0)
        ...
        ;;
      *)
        if [[ "$choice" == "$voice_idx" ]] && [[ "$voice_idx" -gt 0 ]]; then
          voice_recognition_mode
        elif [[ "$choice" == "$adb_idx" ]]; then
          adb_menu
        elif [[ "$choice" == "$config_idx" ]]; then
          modify_config
        elif [[ "$choice" == "$voice_config_idx" ]] && [[ "${voice_config_idx:-0}" -gt 0 ]]; then
          modify_voice_config
        elif [[ "$choice" == "$apps_idx" ]]; then
          list_apps
        elif [[ "$choice" == "$view_idx" ]]; then
          view_config
        elif [[ "$choice" == "$lang_idx" ]]; then
          switch_language
        elif [[ "$choice" == "$uninstall_idx" ]]; then
          uninstall_menu
        else
          echo -e "${RED}$(i18n invalid_choice)${NC}"
          sleep 1
